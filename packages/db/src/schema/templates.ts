...
export const invoiceSchema = {
  ...
  fields: [
    ...
    { name: "paymentStatus", type: "string", required: false, enum: ["paid", "pending", "overdue"], description: "Payment status" },
  ]
};
...
export const bankStatementSchema = {
  ...
  fields: [
    ...
    { name: "amount", type: "number", required: true, description: "Transaction amount (positive for credit, negative for debit)" },
    { name: "transactionType", type: "string", required: true, enum: ["debit", "credit"], description: "Transaction type" },
...
export const productionSalesSchema = {
  ...
  fields: [
    ...
    { name: "fechaAlbaran", type: "date", required: true, description: "Delivery note date (formato: d/M/yyyy)" },
    { name: "numeroAlbaran", type: "string", required: true, description: "Delivery note number (ej: P24 - 24661)" },
    { name: "fechaFactura", type: "date", required: false, description: "Invoice date" },
    { name: "numeroFactura", type: "string", required: false, description: "Invoice number (ej: P25A - 14)" },
    { name: "fechaPago", type: "date", required: false, description: "Payment date" },
    { name: "numeroProducto", type: "string", required: true, description: "Product number/code (ej: 800, 808)" },
    { name: "producto", type: "string", required: true, description: "Product name (ej: Berenjena Violeta, Judia Larga Verde)" },
    { name: "tipoProducto", type: "string", required: false, description: "Product type/quality (Bueno, Estrío, Extra, Primera)" },
    { name: "kgs", type: "number", required: true, description: "Kilograms produced/sold" },
    { name: "precio", type: "number", required: true, description: "Price per kg (decimal con coma en CSV)" },
    { name: "descuento", type: "number", required: false, description: "Discount rate (decimal, ej: 0.1 = 10%)" },
    { name: "facturacionAntesImpuestos", type: "number", required: true, description: "Billing before taxes" },
    { name: "precioAntesImpuestos", type: "number", required: false, description: "Price before taxes per kg" },
    { name: "retencionesPercent", type: "number", required: false, description: "Withholdings percentage (decimal, ej: 0.02 = 2%)" },
    { name: "retencionesEuros", type: "number", required: false, description: "Withholdings amount in euros" },
    { name: "ivaPercent", type: "number", required: false, description: "VAT percentage" },
    { name: "ivaEuros", type: "number", required: false, description: "VAT amount in euros" },
    { name: "facturacionNeta", type: "number", required: true, description: "Net billing amount (after withholdings)" },
    { name: "precioNeto", type: "number", required: false, description: "Net price per kg" },
    { name: "facturacion", type: "number", required: false, description: "Total billing (same as facturacionNeta)" },
    { name: "anomesAlb", type: "string", required: false, description: "Year-month of delivery note (formato: aaYYmm, ej: aa08)" },
    { name: "anomesFra", type: "string", required: false, description: "Year-month of invoice (formato: aaYYmm)" },
    { name: "anomesPago", type: "string", required: false, description: "Year-month of payment (formato: aaYYmm)" },
    { name: "precioPond", type: "number", required: false, description: "Weighted price (precio ponderado calculado)" },
    { name: "precioAntesImpuestosPond", type: "number", required: false, description: "Weighted price before taxes (calculado)" },
    { name: "precioNetoPond", type: "number", required: false, description: "Weighted net price (calculado)" },
  ]
};
